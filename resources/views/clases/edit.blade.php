<!doctype html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1">

        <title>Crear una nueva Clase</title>

        <!-- Fonts -->
        <link href="https://fonts.googleapis.com/css?family=Nunito:200,600" rel="stylesheet" type="text/css">

        <!-- Styles -->
        <style>
            html, body {
                background-color: #fff;
                color: #636b6f;
                font-family: 'Nunito', sans-serif;
                font-weight: 200;
                height: 100vh;
                margin: 0;
            }

            .full-height {
                height: 100vh;
            }

            .flex-center {
                align-items: center;
                display: flex;
                justify-content: center;
            }

            .position-ref {
                position: relative;
            }

            .top-right {
                position: absolute;
                right: 10px;
                top: 18px;
            }

            .content {
                text-align: center;
            }

            .title {
                font-size: 24px;
            }

            .links > a {
                color: #636b6f;
                padding: 0 25px;
                font-size: 13px;
                font-weight: 600;
                letter-spacing: .1rem;
                text-decoration: none;
                text-transform: uppercase;
            }

            .m-b-md {
                margin-bottom: 30px;
            }
        </style>

    </head>
    <body>
        <div class="flex-center position-ref full-height">
            @if (Route::has('login'))
                <div class="top-right links">
                    @auth
                        <a href="{{ url('/home') }}">Home</a>
                    @else
                        <a href="{{ route('login') }}">Login</a>

                        @if (Route::has('register'))
                            <a href="{{ route('register') }}">Register</a>
                        @endif
                    @endauth
                </div>
            @endif

            <div class="content">

            <div class="title m-b-md">
                    Crear Nueva Clase
                </div>
                
                <form method = "POST" action ={{route('clases.update',$clase->id)}} >

                        @csrf
                        {{ method_field('PUT') }}
                        <p><label for="name">
                            Nombre de la Clase 
                            <input type="text" name ="name" value=" {{$clase->name}} ">
                        </label></p>

                        <p><label for="short_name">
                             Nombre Corto 
                            <input type="text" name ="short_name" value=" {{$clase->short_name}} ">
                        </label></p>

                        <p><label for="description">
                             Descripci&oacute;n 
                            <input type="text" name ="description" value=" {{$clase->description}} ">
                        </label></p>

                        <p><label for="semester">
                            Semestre
                             <select name="semester" value=" {{$clase->semester}}" >
                                <option value=0>Curso No semestral</option>
                                <option value=1>Primer Semestre</option>
                                <option value=2>Segundo Semestre</option>
                                
                            </select>
                        </label></p>

                        <p><label for="oficial" value=" {{$clase->oficial}}" >
                            Modalidad
                            <select name="oficial">
                                <option value=1>Si es Oficial</option>
                                <option value=0>No es Oficial</option>
                            </select>
                        </label></p>
                        <p>                          
                            <input type="submit"  value="Enviar">
                        </p>

                </form>
               
            </div>
        </div>
    </body>
</html>
