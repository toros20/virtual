...
//funccion para editar el acumulativo seleccionado
function editar_task(task, course, section){
    
    var token = $("#token").val();

    $.ajax({

        url:'../../../../../../ajax/editar_task',
        headers: token ,
        data: {course_id:course,section_id:section,_token:token,task_id:task},
        type:'POST',
        datatype:'json',
...
//funccion para ver las tareas enviadas por el studiante
function ver_filetasks(clase,task, course, section){
    
    var token = $("#token").val();

    $.ajax({

        url:'../../../../../../ajax/ver_filetasks',
        headers: token ,
        data: {course_id:course,section_id:section,_token:token,task_id:task,class_id:clase},
        type:'POST',
        datatype:'json',
        success:function(data)
        {
            //console.log(response);
            $('#div_modal_ver_filetasks').html(data);
            $('#centralModalver_filetasks').modal({
                show: true
            })
...
function calcular_semestre2(m_id){
    ...
    var examen1 = document.getElementById('exa1_'+m_id).value
        ...
        var t1=eval(acum1)+eval(examen1);
        var t2=eval(acum2)+eval(examen2);
...
function votar_encuesta(pregunta,docente,estudiante,valor){

    var token = $("#token").val();
    var contador = $("#contador").val();
    //almacenamos los valores en la tabla encuesta
    $.ajax({

        url:'../users/votar_encuesta',
        headers: token ,
        data: {_pregunta:pregunta,_docente:docente,_token:token,_estudiante:estudiante,_valor:valor,_contador:contador},
        type:'POST',
        datatype:'json',
        success:function(data)
        {
            //ocultamos la fila de este docente ya evaluado
            $('#div_'+docente).hide();

            //restamos un valor al contador de docentes
            document.getElementById('contador').value=contador-1

            //evaluamos si ya llego al ultimo docente para aparecer el boton de finalizar pregunta
            if (contador-1 ==0){
                $('#div_boton').css('display','inline-block');
            }
