...
class TeacherController extends Controller
{
    ...
    //funcion inicial al panel docente, 
     ...
     function  acumulativos($user_id,$course,$section,$clase,$parcial){
        ...
        /*************************SEGURIDAD*******************/
            ...
            $id_user_log = Auth::id();
            if( $id_user_log != $user_id && $user->role!='coord' ){
                return "ACCESO NO PERMITIDO AL USUARIO ACTUAL."; 
            }
        /*************************SEGURIDAD*******************/
    ...
    function send_file(Request $request){
     ...
     // realizar una publicacion de una imagen (solo para los miembros de una seccion) desde el panel de docente, en la caja de comentarios inicial
     public function post_image_teacher(Request $request){
   ...
   function delete_post(Request $request){
        ...
        $id_users = Enrollment::where([
          ...
          foreach($id_users as $id_user)
          {
            DB::table('msj_'.$id_user->user_id)->where('key', '=', $request->post_key)->delete();
          }
        ...
        foreach($id_users2 as $id_user2)
        {
          DB::table('msj_'.$id_user2->user_id)->where('key', '=', $request->post_key)->delete();
        }
